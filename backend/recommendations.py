...
class Simulation:
    ...
    def find_best_available_position_players(self, available_players, drafting_team):
        ...
        for player in top_target_players:
            ...
            if player_position not in player_position_added:
                ...
                player_position_added.add(player_position)

        return recommended_players, required_positions

    def recommend_player(self, available_players, drafting_team):
        recommended_players, required_positions = self.find_best_available_position_players(available_players, drafting_team)
        ...
        # Find player with lowest rank (highest priority)
        if target_players:
            target_player = min(target_players, key=lambda p: p[1])
    ...
    def predict_available_players(self):
        ...
        # Only copy lists, not objects, for speed
        available_players_one_round = list(self.my_playerboard.players)
        available_players_two_rounds = list(self.my_playerboard.players)
        draft_order_one_round = list(self.my_draft_order)
        draft_order_two_rounds = list(self.my_draft_order)

        for _ in range(picks_to_sim_one_round):
            drafting_team = draft_order_one_round[0]
            recommended_player = self.recommend_player(available_players_one_round, drafting_team)
        ...
        for _ in range(picks_to_sim_two_rounds):
            drafting_team = draft_order_two_rounds[0]
            recommended_player = self.recommend_player(available_players_two_rounds, drafting_team)
            if recommended_player:
